...
_DESTINATION_SYSTEM_INSTRUCTION = """
...
_TRAVEL_SYSTEM_INSTRUCTION = """You are a Travel Meteorologist specializing in weather analysis for travel planning.
...
_CULTURE_SYSTEM_INSTRUCTION ="""You are a Language & Cultural Expert specializing in linguistic and cultural guidance for travelers.
...
async def multi_agent_travel_planner_with_language():
    """
    Multi-agent travel planning system with destination, weather, and language experts
    coordinated via HandoffTool.
    """
    llm = get_llm()
    ...
    travel_meteorologist = RequirementAgent(
        ...
        middlewares=[GlobalTrajectoryMiddleware(included=[Tool])],
        requirements=[
            ConditionalRequirement(ThinkTool, force_at_step=1, min_invocations=1, max_invocations=2),
    ...
    language_and_culture_expert = RequirementAgent(
        ...
        tools=[WikipediaTool(), ThinkTool()],
        memory=UnconstrainedMemory(),
        instructions=_CULTURE_SYSTEM_INSTRUCTION,
        middlewares=[GlobalTrajectoryMiddleware(included=[Tool])],
        requirements=[
            ConditionalRequirement(
                ThinkTool,
